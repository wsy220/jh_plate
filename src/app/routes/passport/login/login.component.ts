...
export class UserLoginComponent implements OnDestroy {
  ...
  submit() {
    ...
    this.http
      .post('api/users/login', {email: this.userName.value, password: this.password.value})
      .subscribe(
        (obj: any) => {
          this.loading = false;
        ...
        response => {
          ...
          if (response.status == 401) {
            this.msg.error(response.error.loginResult)
          } else {
            this.msg.error("用户名密码错误，请输入正确的用户名密码！")
          }
